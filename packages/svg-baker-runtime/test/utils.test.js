...
describe("svg-baker-runtime/utils", () => {
  ...
  describe("getUrlWithoutFragment()", () => {
    it("should work", () => {
      getUrlWithoutFragment("http://www.example.com/#qwe").should.be.equal(
    ...
    it("should return current URL if no arg provided", () => {
      ...
      getUrlWithoutFragment().should.be.equal(
  ...
  describe("moveGradientsOutsideSymbol()", () => {
    ...
    it("should support custom selector", () => {
      test({
        ...
        expected:
          "<defs><mask></mask><symbol><pattern></pattern></symbol></defs>",
        args: ["mask"],
      });
    });
  });
  ...
  describe("stringify()", () => {
    it("should properly serialize DOM nodes", () => {
      ...
      input = wrapInSvgString(
        '<defs><symbol id="foo"></symbol></defs><use xlink:href="#foo"></use>'
      );
      stringify(parse(input)).should.be.equal(input);

      input = wrapInSvgString(
        '<path id="p1"></path><path id="p2"></path><path id="p3"></path>'
      );
      stringify(parse(input)).should.be.equal(input);
    });
  });
  ...
  describe("updateUrls()", () => {
    ...
    it("should replace URLs in attributes and references", () => {
      const input =
        '<linearGradient id="id"></linearGradient><path fill="url(#id)" style="fill:url(#id);"></path><use xlink:href="#id"></use>';
      const expected =
        '<linearGradient id="id"></linearGradient><path fill="url(/prefix#id)" style="fill:url(/prefix#id);"></path><use xlink:href="/prefix#id"></use>';
      const doc = wrapInSvgAndParse(input);

      test({
        input: doc,
        expected,
        args: [doc.querySelectorAll("use"), "#", "/prefix#"],
      });
    });
    ...
    it("should not modify non matched attributes and references", () => {
      ...
      test({
        input: doc,
        expected: input,
        args: [doc.querySelectorAll("use"), "#qwe", "/prefix#qwe"],
      });
    });
  });
});
